...
class Products with ChangeNotifier {
  ...
  Future<void> loaProducts() async {
    final response = await http.get("$baseurl.json?auth=$_token");
   
    Map<String, dynamic> data = json.decode(response.body);
    _items.clear();
    final favResponse = await http.get("${Urls.Base_API}/userFavorites/$_userId.json?auth=$_token");
    ...
    if (data != null) {
      data.forEach((productId, productData) {
        _items.add(Product(
          ...
          isFavorite: favMap == null ? false: favMap[productId] ?? false,
        ));
      });
      notifyListeners();
    }
  ...
  Future<void> updateProduct(Product product) async {
    ...
    if (index >= 0) {
      await http.patch(
        "$baseurl/${product.id}.json?auth=$_token",
        body: json.encode(
          {
            'title': product.title,
            'price': product.price,
